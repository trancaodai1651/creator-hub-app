...
async function publishTikTok(browser, config, replyLog) {
  ...
  try {
    ...
    if (metadata.hashtags) {
      const tags = metadata.hashtags.split(/[\s,]+/).map((t) => t.trim()).filter((t) => t.length > 0);
    ...
    for (let i = 0; i < 150; i++) {
      const state = await page.evaluate((mode) => {
        ...
        if (allText.includes('continue to post') || allText.includes('tiếp tục đăng') || allText.includes('copyright') || allText.includes('bản quyền')) {
          ...
          if (postAnywayBtn && postAnywayBtn.offsetParent !== null) {
            postAnywayBtn.click();
      ...
      if (state.success) { isSuccess = true; break; }
      if (state.clickedCancel) { replyLog(`⚫ [TikTok] Đã gỡ bỏ bảng Exit Modal...`); await new Promise(r => setTimeout(r, 1500)); continue; }
      if (state.clickedPostAnyway) { replyLog(`⚫ [TikTok] Đã xác nhận Đăng ngay (Bỏ qua cảnh báo)...`); await new Promise(r => setTimeout(r, 2000)); continue; }
      ...
      try {
        ...
        if (!currentUrl.includes('/upload')) { isSuccess = true; break; }
      } catch (e) { isSuccess = true; break; }
    ...
    if (!isSuccess) throw new Error(`[Lỗi Timeout] Đã hết thời gian nhưng không thể đăng bài.`);
