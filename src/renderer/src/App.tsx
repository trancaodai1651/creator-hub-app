...
// NẠP ADAPTER ĐỂ GỌI LỆNH TAURI NATIVE
...
export default function App() {
  ...
  // Khởi tạo đầy đủ bộ não của 10 tính năng lớn
  ...
  useEffect(() => { window.electron.ipcRenderer.invoke('get-platform').then((res: string) => setPlatform(res)) }, [])
  ...
  const handleCheckUpdate = async (isManual = false) => {
    try {
      if (isManual) { setCustomModal({ show: true, title: "🔍 ĐANG KIỂM TRA", message: "Hệ thống đang kết nối đám mây GitHub..." }) }
  ...
  return (
    <div className={`flex flex-col h-screen overflow-hidden font-sans transition-colors duration-200 ${colors.c_bgMain} ${isDark ? 'text-zinc-100' : 'text-zinc-800'}`}>
      ...
      {/* VÙNG KÉO THẢ CỬA SỔ NATIVE */}
      <div 
        className="fixed top-0 left-[200px] right-[160px] h-14 select-none bg-transparent"
      ...
      {/* ========================================================== */}
      {/* 1. TOP HEADER TOOLBAR (CỤM NÚT PHẲNG CHUẨN WINDOWS NẰM BÊN TRÁI) */}
      {/* ========================================================== */}
      <header 
        className={`relative z-[9999999] h-14 flex items-center justify-between pl-0 pr-5 border-b select-none shrink-0 transition-all ${
          isDark ? 'bg-[#18181b]/95 border-zinc-800 text-white' : 'bg-white/95 border-zinc-200 text-zinc-900'
        } backdrop-blur-md`} 
        style={{ WebkitAppRegion: 'drag' } as any}
      >
        <div className="flex items-center h-full gap-4">
          
          {/* 3 NÚT ĐIỀU KHIỂN CỬA SỔ PHẲNG HIỆN ĐẠI (ĐÓNG -> PHÓNG TO -> THU NHỎ) */}
        ...
        <div style={{ WebkitAppRegion: 'no-drag' } as any}>
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border font-bold text-xs transition-all shadow-sm ${
              isDark ? 'bg-zinc-800/80 border-zinc-700/60 text-zinc-200 hover:border-red-500/50 hover:bg-zinc-800' : 'bg-white border-zinc-200 text-zinc-700 hover:border-red-400 hover:bg-zinc-50'
            }`}
          >
            <span>⚙️</span>
      ...
      <div className={`flex-1 flex overflow-hidden p-4 gap-4 transition-all ${isDark ? 'bg-[#0f0f12]' : 'bg-zinc-50'}`}>
          ...
          <aside className={`w-[260px] xl:w-[285px] flex flex-col p-3 shrink-0 rounded-[20px] border shadow-sm ${colors.c_bgPanel} ${colors.c_borderT}`}>
            <nav className="flex-1 space-y-1 overflow-y-auto pr-1 custom-scrollbar">
              {SIDEBAR_TABS.map((tab: any) => {
                ...
                return (
                  <button key={tab.id} disabled={isLocked} onClick={() => !isLocked && setActiveTab(tab.id)} 
                    className={`w-full flex items-center gap-3.5 px-3 py-3 rounded-xl font-bold text-sm transition-all ${
                      isLocked ? `opacity-40 cursor-not-allowed ${isDark ? 'text-zinc-500' : 'text-zinc-400'}` : isActive ? 'bg-gradient-to-r from-red-500 to-rose-600 text-white shadow-md shadow-red-500/20 scale-[1.02]' : isDark ? 'text-zinc-300 hover:bg-zinc-800/80 hover:text-white' : 'text-zinc-600 hover:bg-zinc-100'
                    }`}
                  >
                    <span className={`text-lg shrink-0 ${isLocked ? 'grayscale opacity-50' : ''}`}>{tab.icon}</span> 
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden relative">
             ...
             <div className="flex-1 w-full overflow-y-auto custom-scrollbar pr-2 pb-2 flex flex-col">
              <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-5 md:gap-7 w-full p-2">
                {SIDEBAR_TABS.filter((tab: any) => tab.id !== 'home').map((tab: any, index: number) => {
                  const isLocked = tab.isWip;
                  return (
                    <div key={tab.id} onClick={() => !isLocked && setActiveTab(tab.id)}
                      className={`group relative p-6 md:p-7 rounded-[32px] md:rounded-[40px] transition-all duration-500 ease-out flex flex-col gap-4 overflow-hidden border opacity-0 animate-fade-in-up ${
                        isLocked ? `opacity-40 cursor-not-allowed ${isDark ? 'border-zinc-800/30 bg-[#16161a]/30' : 'border-zinc-200/50 bg-zinc-50/50'}` : `cursor-pointer ${isDark ? 'border-zinc-800/80 bg-[#16161a] text-white hover:bg-[#1f1f24]' : 'border-zinc-200 bg-white text-zinc-800 hover:bg-zinc-50'} hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(239,68,68,0.25)] hover:border-red-500/50`
                      }`}
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start relative z-10">
                        <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center transition-colors duration-500 ${isLocked ? isDark ? 'bg-zinc-800/30 grayscale' : 'bg-zinc-200/50 grayscale' : isDark ? 'bg-zinc-800/60 group-hover:bg-red-500/20' : 'bg-zinc-100 group-hover:bg-red-50'}`}>
                        ...
                        {isLocked && <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-amber-500 bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-full shadow-sm backdrop-blur-sm">Đang phát triển</span>}
                      </div>
                      <div className="relative z-10 mt-2">
                        <h3 className={`text-lg md:text-xl font-black transition-colors duration-300 ${isLocked ? (isDark ? 'text-zinc-500' : 'text-zinc-400') : 'text-red-500 group-hover:text-red-400'}`}>{t(tab.nameKey)}</h3>
                        <p className={`text-xs md:text-sm mt-2 leading-relaxed transition-colors duration-300 ${isLocked ? (isDark ? 'text-zinc-600' : 'text-zinc-400') : (isDark ? 'text-zinc-400 group-hover:text-zinc-300' : 'text-zinc-500 group-hover:text-zinc-700')}`}>{t(tab.descKey)}</p>
                      </div>
                    </div>
                  )
                })}
          ...
          {/* ĐIỀU PHỐI ĐA LUỒNG CHO CÁC TAB */}
          {activeTab !== 'home' && (
            <div key={activeTab} className={`w-full h-full flex-1 flex flex-col overflow-hidden opacity-0 animate-fade-in-up rounded-[1.25rem] border shadow-lg ${colors.c_bgPanel} ${colors.c_borderT}`} style={{ animationDuration: '0.3s' }}>
              ...
              {activeTab === 'settings' && <SettingsTab cfg={{ language, setLanguage, themeSetting, setThemeSetting, fontSize, setFontSize, groqKey, setGroqKey, elevenKey, setElevenKey }} t={t} colors={colors} isDark={isDark} onCheckUpdate={() => handleCheckUpdate(true)} />}
            </div>
          )}
        </main>
      </div>
