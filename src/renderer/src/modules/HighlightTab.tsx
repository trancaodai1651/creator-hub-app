    ...
    <div className="flex h-full min-h-0 flex-col gap-4 overflow-hidden p-4 sm:p-5">
      <header className="flex shrink-0 flex-wrap items-end justify-between gap-4">
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-blue-500">Công cụ video</p>
          ...
          <section className={panelClass(isDark)}>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-current/10 pb-4">
              <div>
                ...
                <p className="mt-1 text-xs opacity-55">Có thể chọn nhiều tệp và tạo nhiều đoạn trên cùng một tệp.</p>
              </div>
            ...
            <div className="mt-4 space-y-3">
              ...
              {joiner.highlightSegments.length === 0 && (
                <button type="button" onClick={() => void joiner.chooseHighlightVideos()} className={`flex min-h-[clamp(300px,42vh,560px)] w-full flex-col items-center justify-center rounded-3xl border border-dashed text-center transition hover:border-blue-400 hover:bg-blue-500/5 ${isDark ? 'border-white/15' : 'border-zinc-300'}`}>
          ...
          <aside className="space-y-4">
            <section className={panelClass(isDark)}>
              <div className="border-b border-current/10 pb-4">
                ...
                <p className="mt-1 text-xs opacity-55">Chọn số lượng tệp và khung hình cho đoạn nổi bật.</p>
              </div>
            ...
            <section className={panelClass(isDark)}>
              <div className="border-b border-current/10 pb-4">
                ...
                <p className="mt-1 text-xs opacity-55">Cấu hình riêng cho các tệp đoạn nổi bật đang xuất.</p>
              </div>
              <div className="mt-4 space-y-4">
      ...
      <footer className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-current/10 pt-4">
        <p className="text-xs opacity-55">{joiner.highlightProcessing ? 'Đang xử lý các đoạn nổi bật...' : 'Thứ tự đoạn được giữ nguyên theo danh sách bên trên.'}</p>
        <button type="button" disabled={joiner.highlightProcessing || joiner.highlightSegments.length === 0} onClick={() => void joiner.handleHighlightExport()} className="rounded-2xl bg-blue-500 px-6 py-3 text-xs font-semibold text-white shadow-lg shadow-blue-500/20 transition hover:-translate-y-0.5 hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-40">
          {joiner.highlightProcessing ? 'ĐANG XỬ LÝ...' : 'XUẤT ĐOẠN NỔI BẬT'}
        </button>
      </footer>
    </div>
  )
